...
/**
  *
  */
object GdmPlayerCareerStatsModelAppTask6 {
  def main(args: Array[String]): Unit = {
    ...
    /*  */
    val task5TableName = ConfigUtil.DB_NAME + "." + ConfigUtil.GDM_PLAYER_TASK_5_TEMP

    /* 玩家 top 1 统计 */
    val maxKillsTableName = ConfigUtil.DB_NAME + "." + ConfigUtil.GDM_MAX_KILLS_PLAYER_TEMP
    
    val targetTableName = ConfigUtil.DB_NAME + "." + ConfigUtil.GDM_PLAYER_TASK_6_TEMP

    //分区字段
    val PARTITION_BY = "first_play_date" //first_play_date

    val task5Stats = spark.table(task5TableName)

    val maxKills = spark.table(maxKillsTableName)

    /* 玩家聚合统计 */
    import spark.implicits._
    val task6Stats = task5Stats
      .join(maxKills, maxKills.col("player") === task5Stats.col("player_name"), "left")
...
val name = line.getAs[String]("player_name")
      ...
      TempGdmPlayerTask6(first_play_date, first_play_time, last_play_time, name, total_kills,

...
object GdmPlayerCareerStatsModelAppTask9 {
  def main(args: Array[String]): Unit = {
    ...
    /*  */
    val task8TableName = ConfigUtil.DB_NAME + "." + ConfigUtil.GDM_PLAYER_TASK_8_TEMP

    /* 玩家 top 1 统计 */
    val maxDistRideTableName = ConfigUtil.DB_NAME + "." + ConfigUtil.GDM_MAX_DIST_RIDE_PLAYER_TEMP
    
    val targetTableName = ConfigUtil.DB_NAME + "." + ConfigUtil.GDM_PLAYER_TASK_9_TEMP

    //分区字段
    val PARTITION_BY = "first_play_date" //first_play_date

    val task8Stats = spark.table(task8TableName)

    val maxDistRide = spark.table(maxDistRideTableName)

    /* 玩家聚合统计 */
    import spark.implicits._
    val task9Stats = task8Stats
      .join(maxDistRide, maxDistRide.col("player") === task8Stats.col("player_name"), "left")
    ...
    task9Stats.map(line => {
      ...
      TempGdmPlayerTask9(first_play_date, first_play_time, last_play_time, name, total_kills,
       ...
       max_kills_match, max_assists, max_assists_match, top_10_ratio, max_dist_ride, max_dist_ride_match
      )
